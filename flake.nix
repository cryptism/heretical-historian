{
  ...
  # in its `default` package. Pinned to the exact revision .claude/docs/DESIGN.md
  ...
  outputs = { self, nixpkgs, ghc-wasm-meta }:
    let
      ...
      # Only verified so far against x86_64-linux (.claude/docs/DESIGN.md Decision
    ...
    in
    {
      ...
      apps = forAll (pkgs: {
        ...
        # commands .claude/docs/DESIGN.md Decision 7/33 used to require, now one
      ...
      devShells = forAll (pkgs:
        ...
        in {
          ...
          # `nix develop .#wasm` — everything .claude/docs/DESIGN.md Decision 7/33
