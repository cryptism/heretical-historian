...
The full build-by-build account of `heretical-historian`: what was asked
for, what was rejected, and how each feature was verified against a real
seed rather than just reasoned about. `CLAUDE.md` keeps only a compact
summary of what's built and a lean, actionable work queue; this file is
where the reasoning and verification narrative that used to live inline
in `CLAUDE.md`'s own Status section now lives, moved out because it had
...
**The inter-step day gap now scales with world activity, at the user's
request (work queue item 16).** `advanceEpoch` (`Historian.World`) rolled
a flat uniform `1..300` days per step since Decision 22; it now rolls
within `1..maxGap`, where `maxGap = max 20 (300 - 5 * activity)` and
`activity` is `activeSocieties` count plus their total `livingMembers` —
more and larger cults narrow the range toward a busier 1..20 floor, while
a young, sparse world still gets the original full spread. Still consumes
...
replacement needed — narrowing an existing roll's bounds doesn't add or
remove an RNG draw, so it doesn't reshuffle the cascade the way every
prior RNG-*consumption* change in this project has.

## Work queue item 13, full account: unifying `Dissolved`/`Destroyed` into `Terminated`

Both are now one predicate, `Terminated` (`Historian.Types`), reusing the
*existing* `factAttestedBy :: Maybe EntityId` field to keep the one real
